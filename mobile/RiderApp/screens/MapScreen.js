...
export default function MapScreen({ navigation }) {
  ...
  useEffect(() => {
    if (Platform.OS === 'web') {
      // On web, directly ask for browser geolocation
  ...
  const centerOnUser = () => {
    ...
    if (Platform.OS === 'web' && mapRef.current) {
      // Leaflet flyTo
      mapRef.current.flyTo([currentLocation.latitude, currentLocation.longitude], 16, { duration: 1.5 });
  ...
  if (permissionDenied) {
    return (
      <View style={styles.centered}>
        ...
        <Text style={styles.deniedMessage}>Please enable location access in your settings.</Text>
        {Platform.OS !== 'web' && (
    ...
    <View style={styles.container}>
      ...
      >
        {currentLocation && (
          ...
          >
            {/* Children only used by native Marker; web Marker uses icon in wrapper */}
  ...
  <TouchableOpacity
    ...
    onPress={() => {
      // Fallback: if goBack fails, navigate to Dashboard
...
headerSpacer: {
  width: 44, // balances the back button width
},
});