...
export default function ProfileScreen({ navigation }) {
  ...
  const handleLogout = async () => {
    Alert.alert('Logout', 'Are you sure?', [
      ...
      { text: 'Logout', style: 'destructive', onPress: async () => { await AsyncStorage.clear(); if (typeof window !== 'undefined') window.location.reload(); else navigation.reset({ index: 0, routes: [{ name: 'Login' }] }); } },
    ]);
  };
